	...
	return {
		...
		link: function ($scope, element, attrs) {
			...
			element.bind('click', function () {
				...
				if($scope.openedNavEl)
				{
                    if($scope.openedNavEl != element && $scope.openedNavAttrs.expanded) {
				...
				if(element[0].dataset.name == 'settings')
					return;
				$scope.openedNavEl = element;
                $scope.openedNavAttrs = attrs;

                //$scope.$apply(function(){
                //    $scope.activeNav = element[0].dataset.name;
